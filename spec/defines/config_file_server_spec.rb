...
describe 'ssh::config_file_server' do
  ...
  on_supported_os(redhat).sort.each do |os, os_facts|
    ...
    context "on #{os} with default values for parameters" do
      ...
      it do
        is_expected.to contain_file('/etc/ssh/sshd_config.d/ing.conf').only_with(
          {
            'ensure'  => 'present',
            'owner'   => 'root',
            'group'   => 'root',
    ...
    context "on #{os} with ensure set to valid value" do
      ...
      it { is_expected.to contain_file('/etc/ssh/sshd_config.d/ing.conf').with_ensure('absent') }
    end

    context "on #{os} with owner set to valid value" do
      let(:params) { { owner: 'test' } }

      it { is_expected.to contain_file('/etc/ssh/sshd_config.d/ing.conf').with_owner('test') }
    end

    context "on #{os} with group set to valid value" do
      let(:params) { { group: 'test' } }

      it { is_expected.to contain_file('/etc/ssh/sshd_config.d/ing.conf').with_group('test') }
    end

    context "on #{os} with mode set to valid value" do
      let(:params) { { mode: '0242' } }

      it { is_expected.to contain_file('/etc/ssh/sshd_config.d/ing.conf').with_mode('0242') }
    end
    ...
    context "on #{os} with lines set to valid value" do
      let(:params) do
        {
          lines: {
            ...
            'RevokedKeys'                     => 'test',
            'RDomain'                         => 'test',
            'SecurityKeyProvider'             => '/test/ing.conf',
            'SetEnv'                          => 'test',
      ...
      it { is_expected.to contain_file('/etc/ssh/sshd_config.d/ing.conf').with_content(content_full) }
    end
    ...
    context "on #{os} with custom set to valid value" do
      ...
      content_custom = <<-END.gsub(%r{^\s+\|}, '')
        ...
        |Test2 Value2
      END
      it { is_expected.to contain_file('/etc/ssh/sshd_config.d/ing.conf').with_content(content_custom) }
    end
    ...
    context "on #{os} with lines and custom set to valid value" do
      ...
      content_mix = <<-END.gsub(%r{^\s+\|}, '')
        ...
        |Test2 Value2
      END
      it { is_expected.to contain_file('/etc/ssh/sshd_config.d/ing.conf').with_content(content_mix) }
    end

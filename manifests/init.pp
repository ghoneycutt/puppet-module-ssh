...
class ssh (
  ...
  if is_integer($sshd_config_serverkeybits) == false { fail("ssh::sshd_config_serverkeybits must be an integer and is set to <${sshd_config_serverkeybits}>.") }
  
  if $sshd_config_authorized_keys_file != '.ssh/authorized_keys' {
    validate_absolute_path($sshd_config_authorized_keys_file)
  }

...
class ssh (
  ...
  case $::osfamily {
    ...
    'Suse'    : {
      ...
      case $::architecture {
        'x86_64' : { $default_sshd_config_subsystem_sftp = '/usr/lib64/ssh/sftp-server' }
        'i386'   : { $default_sshd_config_subsystem_sftp = '/usr/lib/ssh/sftp-server' }
        default  : { fail("ssh supports architectures x86_64 and i386 for Suse. Detected architecture is <${::architecture}>.") }
      }
    }
    'Debian'  : {
      $default_packages = [
  ...
  if $sshd_config_serverkeybits_real != undef {
    if is_integer($sshd_config_serverkeybits_real) == false {
      fail("ssh::sshd_config_serverkeybits must be an integer and is set to <${sshd_config_serverkeybits}>.")
    }
  }

  if is_integer($sshd_client_alive_interval) == false {
    fail("ssh::sshd_client_alive_interval must be an integer and is set to <${sshd_client_alive_interval}>.")
  }

  if is_integer($sshd_client_alive_count_max) == false {
    fail("ssh::sshd_client_alive_count_max must be an integer and is set to <${sshd_client_alive_count_max}>.")
  }
  ...
  if $sshd_config_maxstartups != undef {
    validate_re($sshd_config_maxstartups, '^(\d+)+(\d+?:\d+?:\d+)?$', "ssh::sshd_config_maxstartups may be either an integer or three integers separated with colons, such as 10:30:100. Detected value is <${sshd_config_maxstartups}>."
    )
  }
  ...
  case type3x($ssh_config_sendenv_xmodifiers) {
    'string'  : { $ssh_config_sendenv_xmodifiers_real = str2bool($ssh_config_sendenv_xmodifiers) }
    'boolean' : { $ssh_config_sendenv_xmodifiers_real = $ssh_config_sendenv_xmodifiers }
    default   : { fail('ssh::ssh_config_sendenv_xmodifiers type must be true or false.') }
  }
  ...
  case $ssh_key_type {
    'ssh-rsa', 'rsa' : { $key = $::sshrsakey }
    'ssh-dsa', 'dsa' : { $key = $::sshdsakey }
    default          : { fail("ssh::ssh_key_type must be 'ssh-rsa', 'rsa', 'ssh-dsa', or 'dsa' and is <${ssh_key_type}>.") }
  }
  ...
  validate_re($ssh_config_global_known_hosts_mode, '^[0-7]{4}$', "ssh::ssh_config_global_known_hosts_mode must be a valid 4 digit mode in octal notation. Detected value is <${ssh_config_global_known_hosts_mode}>."
  )
  ...
  case type3x($validate_sshd_config) {
    'string'  : {
      validate_re($validate_sshd_config, '^(true|false)$', "ssh::validate_sshd_config may be either 'true' or 'false' and is set to <${validate_sshd_config}>."
      )
    ...
    default   : {
      fail('ssh::validate_sshd_config type must be true or false.')
    }
  }
  ...
  resources { 'sshkey': purge => $purge_keys_real, }
  ...
  if $validate_sshd_config_real == true {
    if versioncmp($::puppetversion, '3.5') >= 0 {
      ...
      validate_cmd(template($sshd_config_template), "${sshd_bin_path} -t -f", 'Sshd failed to validate config content')
    }
